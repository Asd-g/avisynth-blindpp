    ...
    for (int y = 0; y < 4; ++y)
    {
        ...
        if (q != 0)
        {
            ...
            if (aa3_0 < 8 * QP)
            {
                ...
                if (d > 6)
                { /* energy across boundary is greater than in one or both of the blocks */
...
inline void deblock_vert_choose_p1p2(uint8_t* v, int stride, uint64_t* p1p2, int QP)
{
    ...
    ((uint32_t*)&mm_b_qp)[0] = ((uint32_t*)&mm_b_qp)[1] = 0x01010101 * QP;
...
void dering(uint8_t* image, int width, int height, int stride, QP_STORE_T* QP_store, int QP_stride, int chroma)
{
    ...
    for (y = 8; y < height - 8; y += 8)
    {
        for (x = 8; x < width - 8; x += 8)
        {
            ...
            max_diff = QP >> 1;
            max_diffq[0] = max_diffq[1] = max_diffq[2] = max_diffq[3] = max_diffq[4] = max_diffq[5] = max_diffq[6] = max_diffq[7] =
                max_diff;
            ...
            */
            ...
            {
                mov esi,
                    b10x10 // ptr to 10x10 source array
                        lea edi,
                    b8x8filtered // ptr to 8x8 output array
                        mov eax,
                    stride // amt to bump source ptr for next row
                        mov bh,
                    thr mov bl, thr psubusb mm6,
                    mm6 // all 00
                        movd mm7,
                    ebx pshufw mm7, mm7,
                    0 // low word to all words

                    // get compare avg of row -1
                    movq mm3,
                    qword ptr[esi + 1] // save center pixels of line -1

                    movq mm2,
                    mm7 psubusb mm2,
                    qword ptr[esi] // left pixels >= threshold?
                    pcmpeqb mm2,
                    mm6 // ff if >= else 00

                        movq mm5,
                    mm7 psubusb mm5,
                    mm3 // center pixels >= threshold?
                        pcmpeqb mm5,
                    mm6 pavgb mm2,
                    mm5 // combine answers
                        ...
                        movq mm5,
                    mm7 psubusb mm5,
                    qword ptr[esi + 2] // right pixels >= threshold?
                    pcmpeqb mm5,
                    mm6 pavgb mm2,
                    mm5 // combine answers
                    ...
                    movq mm4,
                    qword ptr[esi + 1] // save center pixels
                    ...
                    mm5 // combine answers
                        ...
                        movq mm3,
                    qword ptr[esi + eax + 1] // save center pixels

                    movq mm1,
                    mm7 psubusb mm1,
                    qword ptr[esi + eax] // left pixels >= threshold?
                    pcmpeqb mm1,
                    mm6 // ff if >= else 00
                    ...
                    mm5 // combine answers
                        ...
                        movq mm5,
                    mm7 psubusb mm5,
                    qword ptr[esi + eax + 2] // right pixels >= threshold?
                    pcmpeqb mm5,
                    mm6 pavgb mm1,
                    mm5 // combine answers
                        ...
                        pavgb mm2,
                    mm0 pavgb mm2, mm1 pcmpeqb mm5,
                    mm5 // all ff
                        pcmpeqb mm5,
                    mm2 // 00 or ff
                        pcmpeqb mm5,
                    mm2 // mm2 = ff or 00 ? ff : 00
                        psubusb mm2,
                    mm2 // 00
                        pcmpeqb mm2,
                    mm5 // opposite of mm5
                        pand mm2,
                    mm4 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm2 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
                    ...
                    mm5 // combine answers
                        ...
                        pavgb mm0,
                    mm1 pavgb mm0, mm2 pcmpeqb mm5,
                    mm5 // all ff
                        pcmpeqb mm5,
                    mm0 // 00 or ff
                        pcmpeqb mm5,
                    mm0 // mm2 = ff or 00 ? ff : 00
                        psubusb mm0,
                    mm0 // 00
                        pcmpeqb mm0,
                    mm5 // opposite of mm5
                        pand mm0,
                    mm3 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi + 1 * 8] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm0 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
                    ...
                    mm5 // combine answers
                        ...
                        pavgb mm1,
                    mm0 pavgb mm1, mm2 pcmpeqb mm5,
                    mm5 // all ff
                        pcmpeqb mm5,
                    mm1 // 00 or ff
                        pcmpeqb mm5,
                    mm1 // mm2 = ff or 00 ? ff : 00
                        psubusb mm1,
                    mm1 // 00
                        pcmpeqb mm1,
                    mm5 // opposite of mm5
                    ...
                    mm4 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi + 2 * 8] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm1 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
                    ...
                    mm3 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi + 3 * 8] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm2 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
                    ...
                    mm4 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi + 4 * 8] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm0 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
                    ...
                    mm3 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi + 5 * 8] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm1 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
                    ...
                    mm4 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi + 6 * 8] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm2 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
                    ...
                    mm3 // use orig vales if thresh's diff, else 00
                        pand mm5,
                    qword ptr[edi + 7 * 8] // use filterd vales if thresh's same, else 00
                    por mm5,
                    mm0 // one of them hsa a value
                        movq qword ptr[esi + 1],
                    mm5 // and store 8 pixels
