...
{
    template<typename T>
    [[nodiscard]] AVS_FORCEINLINE int deblock_horiz_useDC_sse(const T* AVS_RESTRICT v, int stride, int moderate_h, int bit_depth) noexcept
    {
        const T* v_0{v};
    ...
    template<typename T>
    AVS_FORCEINLINE void deblock_horiz_lpf9_sse(T* AVS_RESTRICT v, int stride, int QP, int bit_depth, bool is_float_chroma) noexcept
    {
        const T* v_0{v};
        const T* v_1{v + stride};
        const T* v_2{v + 2 * stride};
    ...
    template<typename T>
    AVS_FORCEINLINE void deblock_horiz_default_filter_sse(
        T* AVS_RESTRICT v, int stride, int QP, int bit_depth, bool is_float_chroma) noexcept
    {
        const T* v_0{v};
        const T* v_1{v + stride};
        const T* v_2{v + 2 * stride};
...
template<typename T>
void deblock_horiz_sse(T* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store, int QP_stride,
    int chroma_flag_for_qp, int moderate_h, int bit_depth, bool is_float_chroma) noexcept
{
    for (int x{8}; x < width; x += 8)
    {
        ...
        if (deblock_horiz_useDC_sse<T>(v, stride, moderate_h, bit_depth))
        {
            if (deblock_horiz_DC_on<T>(v, stride, QP, bit_depth))
                deblock_horiz_lpf9_sse<T>(v, stride, QP, bit_depth, is_float_chroma);
        }
        else
            deblock_horiz_default_filter_sse<T>(v, stride, QP, bit_depth, is_float_chroma);
    }
}

template void deblock_horiz_sse<uint8_t>(uint8_t* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride, int chroma_flag_for_qp, int moderate_h, int bit_depth, bool is_float_chroma) noexcept;
template void deblock_horiz_sse<uint16_t>(uint16_t* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride,
    int chroma_flag_for_qp, int moderate_h, int bit_depth, bool is_float_chroma) noexcept;
template void deblock_horiz_sse<float>(float* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride,
    int chroma_flag_for_qp, int moderate_h, int bit_depth, bool is_float_chroma) noexcept;
...
{
    template<typename T>
    [[nodiscard]] AVS_FORCEINLINE int deblock_vert_useDC_sse(const T* AVS_RESTRICT v, int stride, int moderate_v, int bit_depth) noexcept
    {
        if constexpr (std::is_floating_point_v<T>)
        {
            const float thr{1.0f / 255.0f};
    ...
    AVS_FORCEINLINE void deblock_vert_lpf9_sse(const T_intermediate* AVS_RESTRICT v_local, const T_intermediate* AVS_RESTRICT p1p2,
    ...
    template<typename T>
    AVS_FORCEINLINE void deblock_vert_default_filter_sse(
        T* AVS_RESTRICT v, int stride, int QP, int bit_depth, bool is_float_chroma) noexcept
    {
        if constexpr (std::is_floating_point_v<T>)
        {
...
template<typename T>
void deblock_vert_sse(T* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store, int QP_stride,
    int chroma_flag_for_qp, int moderate_v, int bit_depth, bool is_float_chroma) noexcept
{
    ...
    for (int Bx{0}; Bx < width; Bx += 8)
    {
        ...
        if (deblock_vert_useDC_sse<T>(v, stride, moderate_v, bit_depth))
        {
            if (deblock_vert_DC_on<T>(v, stride, QP, bit_depth))
            {
                ...
                deblock_vert_lpf9_sse<T, intermediate_t>(v_local.data(), p1p2.data(), v, stride, bit_depth, is_float_chroma);
            }
        }
        else
            deblock_vert_default_filter_sse<T>(v, stride, QP, bit_depth, is_float_chroma);
    }
}

template void deblock_vert_sse<uint8_t>(uint8_t* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride, int chroma_flag_for_qp, int moderate_v, int bit_depth, bool is_float_chroma) noexcept;
template void deblock_vert_sse<uint16_t>(uint16_t* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride, int chroma_flag_for_qp, int moderate_v, int bit_depth, bool is_float_chroma) noexcept;
template void deblock_vert_sse<float>(float* AVS_RESTRICT image, int width, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride,
    int chroma_flag_for_qp, int moderate_v, int bit_depth, bool is_float_chroma) noexcept;

template<typename T>
void dering_sse(T* AVS_RESTRICT image, int width, int height, int stride, const QP_STORE_T* AVS_RESTRICT QP_store, int QP_stride,
...
template void dering_sse<uint8_t>(uint8_t* AVS_RESTRICT image, int width, int height, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride, int chroma_flag_for_qp, int bit_depth, bool is_float_chroma) noexcept;
template void dering_sse<uint16_t>(uint16_t* AVS_RESTRICT image, int width, int height, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride, int chroma_flag_for_qp, int bit_depth, bool is_float_chroma) noexcept;
template void dering_sse<float>(float* AVS_RESTRICT image, int width, int height, int stride, const QP_STORE_T* AVS_RESTRICT QP_store,
    int QP_stride, int chroma_flag_for_qp, int bit_depth, bool is_float_chroma) noexcept;
