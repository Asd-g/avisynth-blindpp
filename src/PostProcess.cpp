...
template<typename T>
using deblock_h_func_t = void (*)(T*, int, int, const QP_STORE_T*, int, int, int, int, bool) noexcept;
template<typename T>
using deblock_v_func_t = void (*)(T*, int, int, const QP_STORE_T*, int, int, int, int, bool) noexcept;
template<typename T>
using dering_func_t = void (*)(T*, int, int, int, const QP_STORE_T*, int, int, int, bool) noexcept;
...
void postprocess_impl(FrameData& frame, const PostProcessConfig& config) noexcept
{
    ...
    struct processors
    {
        deblock_h_func_t<T> horiz;
        deblock_v_func_t<T> vert;
        dering_func_t<T> dering;
    };
    ...
    for (int plane_idx{0}; plane_idx < num_planes; ++plane_idx)
    {
        ...
        for (int y{0}; y < v_size; y += 4)
        {
            ...
            if ((mode & deblock_h_flag) == deblock_h_flag)
            {
                ...
                if (!iPP)
                {
                    deblock_horiz_process(p_dst + static_cast<ptrdiff_t>(y) * dst_stride, plane_width, dst_stride, qp_row, QP_stride,
                        chroma_flag_for_qp, moderate_h, bit_depth, is_float_chroma);
                }
                else
                {
                    deblock_horiz_process(p_dst + static_cast<ptrdiff_t>(y) * 2 * dst_stride, plane_width, dst_stride * 2,
                        &QP_store[(y >> qp_y_shift) * 2 * QP_stride], QP_stride * 2, chroma_flag_for_qp, moderate_h, bit_depth,
                        is_float_chroma);
                    deblock_horiz_process(p_dst + (static_cast<ptrdiff_t>(y) * 2 + 1) * dst_stride, plane_width, dst_stride * 2,
            ...
            if ((mode & deblock_v_flag) == deblock_v_flag)
            {
                if (y >= 12 && (y % 8) == 4)
                {
                    ...
                    if (!iPP)
                    {
                        deblock_vert_process(p_dst + static_cast<ptrdiff_t>(boundary_y) * dst_stride, plane_width, dst_stride, qp_row,
                            QP_stride, chroma_flag_for_qp, moderate_v, bit_depth, is_float_chroma);
                    }
                    else
                    {
                        deblock_vert_process(p_dst + static_cast<ptrdiff_t>(boundary_y) * 2 * dst_stride, plane_width, dst_stride * 2,
                            &QP_store[(boundary_y >> qp_y_shift) * 2 * QP_stride], QP_stride * 2, chroma_flag_for_qp, moderate_v, bit_depth,
                            is_float_chroma);
                        deblock_vert_process(p_dst + (static_cast<ptrdiff_t>(boundary_y) * 2 + 1) * dst_stride, plane_width, dst_stride * 2,
        ...
        if ((mode & dering_flag) == dering_flag)
        {
            ...
            {
                dering_process(
                    p_dst, plane_width, plane_height, dst_stride, QP_store, QP_stride, chroma_flag_for_qp, bit_depth, is_float_chroma);
            }
            else
            {
                dering_process(
                    p_dst, plane_width, v_size, dst_stride * 2, QP_store, QP_stride * 2, chroma_flag_for_qp, bit_depth, is_float_chroma);
                dering_process(p_dst + dst_stride, plane_width, v_size, dst_stride * 2, QP_store + QP_stride, QP_stride * 2,
                    chroma_flag_for_qp, bit_depth, is_float_chroma);
            }
        }
    }
}
